...
# script: rook_integra-ad_UBUNTU.sh
...
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Módulo 2 – Instalação de Pacotes Essenciais - ajustado em 2026-05-08 
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
mod2_install() {
    ...
    if [ "$UPGRADABLE_COUNT" -gt 0 ]; then
        ...
        echo "    --------------------------------------------------------------------"
        echo "        Recomenda-se aplicar as atualizações para evitar problemas."
        echo "        (execute: 'sudo apt upgrade -y' após o script ou agora)"
        echo "    ===================================================================="
        echo ""
        
        while true; do
            read -erp "    Deseja continuar mesmo assim? (s/n): " CONTINUAR_MESMO_ASSIM
            CONTINUAR_MESMO_ASSIM=$(echo "$CONTINUAR_MESMO_ASSIM" | tr '[:upper:]' '[:lower:]')
            if [[ "$CONTINUAR_MESMO_ASSIM" == "s" ]]; then
                echo "    [!] Continuando o script com sistema desatualizado..."
                sleep 2
                break
            elif [[ "$CONTINUAR_MESMO_ASSIM" == "n" ]]; then
                echo "    [X] Operação abortada pelo usuário. Execute 'sudo apt upgrade -y' e depois rode o script novamente."
                return 1
            else
                echo "    Resposta inválida. Por favor, digite 's' para sim ou 'n' para não."
            fi
    ...
    echo "        adcli chrony $DNS_PKG ldap-utils libnss-sss libpam-sss"
    echo "        oddjob oddjob-mkhomedir packagekit realmd samba-common-bin"
    echo "        sssd sssd-tools"
    sleep 1.5

    # Instala os pacotes de forma resiliente
    if ! sudo DEBIAN_FRONTEND=noninteractive apt install -y \
        ...
        sssd \
        sssd-tools; then
        echo "    [X] ERRO: Falha na instalação de pacotes."
        return 1
    fi
    ...
    # Lista de pacotes críticos para validação (EXCETO dnsutils)
    local CRITICAL_PACKAGES_ARRAY=(
        adcli chrony ldap-utils libnss-sss libpam-sss \
        oddjob oddjob-mkhomedir packagekit realmd samba-common-bin \
        sssd sssd-tools
    )

    # Validação padrão via dpkg
    ...
    # ==========================
    # VALIDAÇÃO FUNCIONAL DNS
    # ==========================
    if command -v dig >/dev/null 2>&1; then
        echo "    [OK] dnsutils (dig disponível)"
    else
        echo "    [X] dnsutils ESTÁ FALTANDO! (dig não encontrado)"
        ALL_INSTALLED=false
    fi
    ...
    if $ALL_INSTALLED; then
        echo "    [!] SUCESSO - Todos os pacotes estão instalados."
        sleep 0.5
        return 0
    else
        echo "    [X] FALHA - PACOTES CRÍTICOS ESTÃO FALTANDO!"
        echo "        Verifique a rede e tente novamente."
        sleep 0.5
        return 1
    fi
}
...
mod3_adinfo() {
    ...
    # ------------------------------------------------------------------------------------------
    # Armazena o hostname atual (apenas o nome curto, sem domínio)
