...
#
# Versao: 7.2 Odometer
# Alteracao: 2026-06-12 (Inclusao de Atributos estendidos de S.O. e DNS para Computadores)
# ==============================================================================
    ...
    if (Test-Path $CSV_Comps) {
        ...
        Import-Csv $CSV_Comps | ForEach-Object {
            ...
            if (!(Get-ADComputer -Filter "SamAccountName -eq '$($obj.Name.Trim())$'" -ErrorAction SilentlyContinue)) {
                ...
                # Isolamento do Hostname curto e concatenacao dinamica com o novo dominio
                $CleanHostname = $obj.Name.Trim().Split('.')[0]
                $TargetDNSHostName = "$CleanHostname.$domain_name"

                $compProps = @{ 
                    Name = $CleanHostname
                    Path = $TargetPath 
                    Enabled = ($obj.Enabled -eq 'True') 
                    Description = $obj.Description 
                    Location = $obj.Location 
                    DNSHostName = $TargetDNSHostName
                    OperatingSystem = $obj.OperatingSystem
                    OperatingSystemVersion = $obj.OperatingSystemVersion
                    OperatingSystemServicePack = $obj.OperatingSystemServicePack
                }
                
                if($i){ try { New-ADComputer @compProps -ErrorAction Stop; $CP_OK++ } catch { $CP_Err++; "[COMP] Erro $($CleanHostname): $($_.Exception.Message)" | Out-File $ArquivoErroLog -Append } }
                elseif ($s) { $CP_OK++ }
            } else { $CP_OK++ }
        }
    }
