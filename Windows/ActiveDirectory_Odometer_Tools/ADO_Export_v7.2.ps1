...
$CPCount = 0
Get-ADComputer -Filter * -Properties DNSHostName, OperatingSystem, OperatingSystemVersion, OperatingSystemServicePack, IPv4Address, Enabled, Description, Location | ForEach-Object {
    $CPCount++; Set-Odometer $CPCount; $_
} | Select-Object Name, DNSHostName, SamAccountName, OperatingSystem, OperatingSystemVersion, OperatingSystemServicePack, IPv4Address, Enabled, DistinguishedName, Description, Location | Export-Csv -Path "$CaminhoExport\02_Computadores.csv" -NoTypeInformation -Encoding UTF8
